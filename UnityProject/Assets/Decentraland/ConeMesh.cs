...
namespace Dcl
{
	...
	// the top center is placed at (0/length/0)
	// the bottom center is placed at (0/0/0)
	...
	public class ConeMesh : ScriptableWizard {
		...
		static private Mesh s_mesh = null;

//		[MenuItem ("GameObject/Create Other/Cone")]
//		static void CreateWizard()
//		{
//			ScriptableWizard.DisplayWizard("Create Cone", typeof(ConeMesh));
//		}
		...
		void OnWizardCreate(){
			...
			string meshName = "Cone" + numVertices + "v" + radiusTop + "t" + radiusBottom + "b" + length + "l" + length + (outside?"o":"") + (inside?"i":"");
			string meshPrefabPath = "Assets/Decentraland/Internal/" + meshName + ".asset";
			s_mesh = (Mesh)AssetDatabase.LoadAssetAtPath(meshPrefabPath, typeof(Mesh));
			if(s_mesh==null){
				s_mesh=new Mesh();
				s_mesh.name=meshName;
				...
				for(i=0;i<numVertices;i++){
					...
					vertices[i+numVertices]=new Vector3(radiusBottom*angleCos,0,radiusBottom*angleSin);
				...
				s_mesh.triangles = tris;		
				AssetDatabase.CreateAsset(s_mesh, meshPrefabPath);
				AssetDatabase.SaveAssets();
			}

			DclPrimitiveHelper.CreateDclPrimitive (DclPrimitiveType.cone);
